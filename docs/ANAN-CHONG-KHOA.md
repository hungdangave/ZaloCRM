...
| 6.2 Rate limit + delay giống người | CÓ SẴN + **BỔ SUNG** | Trần daily/burst per-nick per-category đã có (SdkLimit, cấu hình trên UI). Bổ sung `send-pacing.ts`: 2 tin liên tiếp cùng số cách nhau ≥1,5s + jitter 0–2,5s (≈18–40 tin/phút); kết bạn nghỉ 20–45s. Env: `ZALO_PACE_*`, tắt bằng `ZALO_PACE_DISABLED=1`. |
...
1. Mua proxy **residential VN, sticky**, tối thiểu = số nick (30+). Mỗi số 1 proxy CỐ ĐỊNH, không xoay.
2. Điền proxy cho **từng** nick trong UI Quản lý tài khoản Zalo (dạng `http://user:pass@ip:port` hoặc `socks5://...`) — **trước khi quét QR**.
